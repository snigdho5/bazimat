...
class Helpers
{
    ...
    public static function product_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item['avg_rating'] = (float)(!empty($item->rating[0]) ? $item->rating[0]->average : 0);
                $item['rating_count'] = (int)(!empty($item->rating[0]) ? $item->rating[0]->rating_count : 0);
            ...
            $data['rating_count'] = (int)(!empty($data->rating[0]) ? $data->rating[0]->rating_count : 0);
    ...
    public static function get_restaurant_discount($restaurant)
    {
        if ($restaurant->discount) {
            ...
            if ($restaurant->opening_time->format('H:i') <= now()->format('H:i') && $restaurant->closeing_time->format('H:i') >= now()->format('H:i'))

            // if($restaurant->start_date->format('Y-m-d') <= now()->format('Y-m-d') && $restaurant->end_date->format('Y-m-d') >= now()->format('Y-m-d') && $restaurant->opening_time->format('H:i') <= now()->format('H:i') && $restaurant->closeing_time->format('H:i') >= now()->format('H:i'))
            {
                return [
                    'discount' => $restaurant->discount,
                    'min_purchase' => $restaurant->minimum_order,
                    // 'max_discount'=>$restaurant->max_discount
                ];
            }
        }
        return null;
    ...
    public static function send_order_notification($order)
    {
        ...
        try {
            ...
            if ($order->order_type == 'delivery' && !$order->scheduled && $order->order_status == 'pending' && config('order_confirmation_model') == 'deliveryman' && ($order->payment_method == 'cash_on_delivery' || $order->payment_method == 'digital_payment')) {
                // echo 'ss2';
                $data = [
                    'title' => trans('messages.order_push_title'),
            ...
            // echo '>>'.$order->delivery_man->fcm_token;
            // print_r($order);
            // die;
            return true;
        } catch (\Exception $e) {
