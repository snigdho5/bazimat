...
class ConversationController extends Controller
{
    public function messages_store(Request $request)
    {
        ...
        return response()->json(['state'=>0,'message' => 'successfully sent!'], 200);
    }
    ...
    {
        $val=Conversation::where(['user_id' => $request->user()->id])->latest()->get();
        //return response()->json([Conversation::where(['user_id' => $request->user()->id])->latest()->get()], 200);
        return response()->json(['state'=>0, 'errors'=>$val], 200);
    }
}
