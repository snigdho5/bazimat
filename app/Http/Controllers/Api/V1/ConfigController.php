...
class ConfigController extends Controller
{
    ...
    public function configuration_delivery()
    {
        return response()->json([
            'terms_and_conditions' => BusinessSetting::where(['key' => 'terms_and_conditions'])->first()->value,
            'privacy_policy' => BusinessSetting::where(['key' => 'privacy_policy'])->first()->value,
    ...
    public function get_zone(Request $request)
    {
        ...
        if(count($zones)<1)
        {
            return response()->json(['state' => 1,'message'=>trans('messages.service_not_available_in_this_area')], 200);
        }
        foreach($zones as $zone)
        {
            if($zone->status)
            {
                return response()->json(['state' => 0,'zone_id'=>$zone->id], 200);
            }
        }
        return response()->json(['state' => 1,'message'=>trans('messages.we_are_temporarily_unavailable_in_this_area')], 200);
    }
